...
from sqlalchemy import ColumnElement, CursorResult, delete, func, literal, select
...
class RequestLogRepository:
    ...
    async def count(self, query: RequestLogQuery) -> int:
        bounded = (
            select(literal(1))
            .select_from(RequestLog)
            .where(*_where(query))
            .limit(MAX_COUNTED_ROWS + 1)
            .subquery()
        )
        result = await self._session.execute(select(func.count()).select_from(bounded))
