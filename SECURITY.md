...
Error responses never echo what was submitted. The validation handler in `app/core/http/errors.py` keeps `type`, `loc`, and `msg` and drops `input`, because FastAPI's default 422 body repeats the rejected value, which would put passwords into response bodies and access logs. Unexpected exceptions return a flat 500 and log the traceback server side, so stack traces and connection strings stay internal.
...
An inbound `X-Request-ID` is filtered before it reaches a log line. `app/core/observability/request_context.py` accepts at most 64 characters of `[A-Za-z0-9_-]` and mints a fresh ID for anything else, because the header is caller-controlled and a newline in it would let that caller write forged entries into the log stream.

Request logs record metadata only. The access line in `app/core/observability/request_logging.py` carries method, path, status, duration, client IP, correlation ID, and the authenticated user id. Bodies, query values, and headers never reach it. That is deliberate: a redaction list is the kind of thing nobody remembers to update, and there is nothing here for one to cover.
...
A password change ends the sessions already open. `users.password_changed_at` is stamped by `AuthService.change_password` and by a completed reset, and `get_actor` refuses any access token issued before it, so the half-hour window an access token used to carry after a password change is closed.
...
`POST /auth/password/forgot` answers 202 with an empty body whether or not the address has an account, and answers before deciding which of the two it is: the lookup, the token and the message all run in a background task on their own session, so the two branches are indistinguishable by response time. `MAIL_RESEND_COOLDOWN_SECONDS` bounds the other half of the problem, since it is the recipient rather than the caller that a flood of links would land on.

Retried writes are answered rather than repeated. A client that sends `Idempotency-Key` gets the stored response back on a retry, and the key is scoped to the account that sent it: `IdempotencyMiddleware` verifies the bearer token's signature before taking the caller's id, so one account is never answered with another's stored response. A request with no valid token ignores the header, and so does a borrowed session, because two people writing as one account would otherwise share a key space. A claim whose process died is taken over once `IDEMPOTENCY_IN_FLIGHT_TIMEOUT_SECONDS` has passed, so a crash costs a retry rather than the key.
