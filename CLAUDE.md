...
- **A repository extends `BaseRepository`.** It already holds the session and the flush, refresh, rowcount and batch-delete plumbing, so a new repository writes queries and nothing else. Do not re-add a `__init__`, a `cast(CursorResult[Any], ...)` or a hand-rolled batch delete. Override `_flush` when a write needs its own failure translation, the way `ItemRepository` turns a `StaleDataError` into a 412.
