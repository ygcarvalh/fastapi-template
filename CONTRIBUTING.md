...
There is no CI workflow, so those five commands are the whole gate. Install the hooks once with `uv run pre-commit install` and ruff and mypy run on commit, which catches most of it earlier.
