...
Those are the same five commands CI runs. Install the hooks once with `uv run pre-commit install` and ruff and mypy run on commit, which catches most of it earlier.
...
Conventional Commits, header only, imperative mood, no trailing period. Allowed types: `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`, `perf`. Put the reasoning in the pull request instead of the commit body. Keep 50 characters or fewer by hand; CI checks the type prefix rather than the length, so that Dependabot's longer subjects still pass.
...
`CHANGELOG.md` is generated from the commit history with [git-cliff](https://git-cliff.org), configured under `[tool.git-cliff]` in `pyproject.toml`. CI does not diff it against a fresh generation. Pull requests are squash merged, so the squashed subject replaces the branch subjects the file was written from, and a check like that can never pass.
...
Pushing a `v*` tag runs `.github/workflows/release.yml`, which regenerates the notes for that tag alone and opens a GitHub release with them. Versioning is semver, and while the major stays at 0 a breaking change to the template's layout or configuration is a minor bump.
